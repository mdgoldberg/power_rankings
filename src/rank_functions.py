...
def expected_wins(df, start_week, end_week):
    ...
    for wk, group in df.groupby('week'):
        for p in players:
            pScore = group.ix[group.team == p, 'score'].values[0]
            allScores = group.ix[:, ['team', 'score']].values
...
def get_wins(df, start_week, end_week):
    df = df.query('@start_week <= week <= @end_week')
    return dict(df.groupby('team').wins.sum().iteritems())
...
def projected_wins(df, start_week, end_week):
    ...
    return {
        ...
        for (tm, wins), loss in zip(total_wins.iteritems(), total_losses)
    }
...
def points_for(df, start_week, end_week):
    points = Counter()
    df = df.query('@start_week <= week <= @end_week')
    for k, v in df.groupby('team').score.sum().iteritems():
        points[k] = v
    return points
